...
getFileListData().then((result) => {
  ...
  textBinding(pathArr, fileNameArr);
}); // getFileListData 끝



//^ 텍스트 데이터를 화면에 바인딩
async function textBinding(pathArr, fileNameArr) {
  console.log(pathArr);
  ...
  fileNameArr.forEach((element, index) => {
    ...
    tbody.append(temp);
    document.getElementById(`fileData${index}`).innerHTML = `${index + 1}.  ${element}`;
...
// //# video.js 와 a-video
// const sub_video = document.querySelectorAll(".video--sub");
