...
class App {
    constructor() {
        ...
        // renderer.setClearColor("#343549"); // 배경색.
    ...
    _createGuiControls() {
        ...
        //=> 서버로 보낼 센서값들을 저장하기 위한 어레이
        const sensorValue = [90, 90, 90, 90, 90, 180];
        ...
        gui.add(this.arm1Rot, 'y', 0, 180, 1).name('arm1 Y').onChange(() => {
            ...
            if (this.arm1) {
                ...
                // sensorValue[0] = THREE.MathUtils.radToDeg(this.arm1.rotation.y) //=> 라디안 -> 각도로 변환 후 + 90 후 배열에 저장
                sensorValue[0] = this.arm1Rot.y;
            }
            this.sendData(sensorValue);    //=> 데이터 보내주는 역할의 함수 실행
        }).setValue(90);    //=> 초기값 지정
        ...
        gui.add(this.arm2Rot, 'x', 0, 180, 1).name('arm2 X').onChange(() => {
            if (this.arm2) {
                ...
                // sensorValue[1] = THREE.MathUtils.radToDeg(this.arm2.rotation.x)
                sensorValue[1] = this.arm2Rot.x;
            }
            this.sendData(sensorValue);
        ...
        gui.add(this.arm3Rot, 'x', 0, 180, 1).name('arm3 X').onChange(() => {
            if (this.arm3) {
                ...
                // sensorValue[2] = THREE.MathUtils.radToDeg(this.arm3.rotation.x)
                sensorValue[2] = this.arm3Rot.x;
            }
            this.sendData(sensorValue);
        }).setValue(90); // Set default value to 90 degrees
        ...
        gui.add(this.arm4Rot, 'x', 0, 180, 1).name('arm4 X').onChange(() => {
            if (this.arm4) {
                ...
                // sensorValue[3] = THREE.MathUtils.radToDeg(this.arm4.rotation.x)
                sensorValue[3] = this.arm4Rot.x;
            }
            this.sendData(sensorValue);
        }).setValue(90); // Set default value to 90 degrees
        ...
        gui.add(this.hand01Rot, 'y', 0, 180, 1).name('hand5 Y').onChange(() => {
            ...
            if (this.hand01) {
                ...
                // sensorValue[4] = THREE.MathUtils.radToDeg(this.hand01.rotation.y)
                sensorValue[4] = this.hand01Rot.y;
            }
            this.sendData(sensorValue);
        }).setValue(90);
        ...
        gui.add(this.hand11LRot, 'z', 0, 180, 1).name('hand Grab').onChange(() => {
            ...
            if (this.hand11L) {
                ...
                // sensorValue[5] = THREE.MathUtils.radToDeg(this.hand11LRot.z)
                sensorValue[5] = this.hand11LRot.z;
            }
            this.sendData(sensorValue);
        }).setValue(180);
    }
    ...
    sendData(sensorValue) {
        const URL = `https://idb.p-e.kr:5000/control?sensor1=${sensorValue[0]}&sensor2=${sensorValue[1]}&sensor3=${sensorValue[2]}&sensor4=${sensorValue[3]}&sensor5=${sensorValue[4]}&sensor6=${sensorValue[5]}`;
