...
/**
 *^ A-Frame용 지형 히트맵/하이트맵 모델 구성 요소입니다.
 */
 AFRAME.registerComponent('aframe-heatmap3d', {
  ...
  schema: {
    ...
    palette: {
      type: 'string',
      default: 'redblue' // Color Brewer에서 가져왔습니다. 유효한 JSON 문자열이어야 합니다. (readable by JSON.parse())
    },
    flipPalette: {
      type: 'boolean',
      default: false
  ...
  customVertexShader: '' +
    '     attribute float opacity;' + // 이 정점의 원하는 불투명도
    '     attribute float height01;' + // 이 정점의 원하는 높이
//    '     varying vec4 vColor;' +
...
*/
  ...
  /**
   * ^ 구성 요소가 연결될 때와 구성 요소 데이터가 변경될 때 호출됩니다.
   * ^ 일반적으로 데이터를 기반으로 엔터티를 수정합니다.
   */
  update: function (oldData) {
    ...
    var el = this.el;
    var data = this.data;
    var vi, ci; // 정점 인덱싱을 위해 for 루프에 사용됨

    data.src = data.src || ''; // 사용자가 지정하지 않으면 '. 여기를 수정하십시오.
    ...
    /*
     * 여기에서 소스 이미지 로드(캔버스로 렌더링) 및 StackBlur 적용을 처리합니다.
     * 그런 다음 준비가 되면 update()에 대한 콜백을 발행합니다.
     */
    if (data.src === '') return;
    ...
    if ("src" in diff || "srcMobile" in diff) { //} && (data.src.length>0 || data.srcMobile.length>0)) {
      ...
      console.log("img", img)
      // 이것은 다양한 버전의 AFrame에서 다르게 처리됩니다.
...
//        document.body.appendChild(data.canvas);
        ...
        thisComponent.update(data);  // update()를 다시 실행하여 아래 코드를 실행하고 실제로 지형 메시를 생성할 수 있습니다.
      }// onImageLoaded
    } // "src" in diff?
    ...
    /*
     * 언로드할 기존 지오메트리가 있는 경우 언로드 애니메이션을 실행합니다.
      * 버튼 클릭시 자연스러운 애니메이션 처리를 하는 부분
     */
     ...
     * 팔레트 문자열을 색상 배열로 변환
     * 내장 팔레트도 여기에 넣습니다.
     * 셰이더에서 사용하기 위해 팔레트를 기반으로 1xN THREE.Texture를 만듭니다.
     */
    ...
    */
    ...
    if (data.updateGeometry) {
          ...
          // 이미지 데이터에서 Red 값을 읽어 각 픽셀의 높이 값을 얻습니다.
          ...
          // 데이터를 전달하여 최대값과 최소값을 얻습니다.
          // 여기에서 invertElevation 작업도 수행합니다. 일반적으로 검정색=100% 고도 및 흰색=0 고도
          this.maxPixelVal=0;
          ...
          //^ 이미지 데이터에서 높이 추출
          // 이미지에서 white=255 RGB이지만 고도는 0이고 black=0=1 고도입니다.
    ...
    /*
     이제 최종적으로 정점 높이, 색상 및 불투명도를 계산할 수 있습니다.
     */
    var clr;
    var di = 0; // 고도 데이터에 색인을 생성합니다. 또한 정점당 1개의 값만 있는 속성을 인덱싱하는 데 사용됩니다(위치와 같이 3개의 값이 아님).
    var val=0; // 표고 DEM의 값
    var NONZEROVERTS=0;
    ...
    if (data.updateMaterial || data.updateGeometry ) {
      this.time_material = window.tic();
        ...
        // D3의 색상 매핑 기능을 사용하여 값을 색상 팔레트에 매핑
        ...
        /*
         * 재료를 만듭니다. 정점별 불투명도를 사용하려면 사용자 정의 셰이더 재료를 사용해야 합니다.
         * See: https://github.com/mrdoob/three.js/issues/2118
         * 투명도가 없으면 일반 Lambert 재질을 사용할 수 있습니다.
         */
    ...
    /*
     * 이제 지오메트리를 만들었으므로 화려한 수직 스케일링 애니메이션을 수행할 수 있습니다.
     */
